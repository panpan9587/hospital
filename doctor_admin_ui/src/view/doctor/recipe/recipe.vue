<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="就诊人" prop="userId" width="120">
        ...
        <el-table-column align="left" label="处方用药" prop="drug" width="120" />
        <el-table-column align="left" label="处方价格" prop="price" width="120" />
        <el-table-column align="left" label="开药医生" prop="prescriber" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRecipeFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getRecipeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRecipeByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRecipeFunc = async(row) => {
    const res = await findRecipe({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRecipeFunc = async (row) => {
    const res = await deleteRecipe({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
