<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="预约id" prop="appointmentId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHealthFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="预约id:"  prop="appointmentId" >
              <el-input v-model.number="formData.appointmentId" :clearable="true" placeholder="请输入预约id" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getHealthList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteHealthByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHealthFunc = async(row) => {
    const res = await findHealth({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHealthFunc = async (row) => {
    const res = await deleteHealth({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
