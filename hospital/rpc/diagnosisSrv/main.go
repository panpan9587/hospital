...
import (
	...
	"demo/rpc/diagnosisSrv/etc"
	"demo/rpc/diagnosisSrv/service"
	"demo/zaplog"
	"fmt"
	"go.uber.org/zap"
...
func main() {
	...
	//初始化数据库
	s := grpc.NewServer()
	proto.RegisterDiagnosisServer(s, service.Diagnosis{})
	...
	zap.S().Info("Diagnosis Srv 服务启动", etc.RpcConfig.Server.Port)
	err = s.Serve(lis)
	if err != nil {
		zap.S().Info("Diagnosis Srv 服务启动失败")
	}
}
