...
type Appointment struct {
	...
	PackageId       int     `gorm:"type:int(11);not null" json:"package_id"`
	Package         Package `gorm:"foreignKey:Id;references:PackageId"`
	gorm.Model
}
...
func GetAppointmentPackageId(db *gorm.DB, id int) (*Appointment, error) {
	appointment := new(Appointment)
	return appointment, db.Preload("Package").Where("id = ?", id).First(appointment).Error
}
